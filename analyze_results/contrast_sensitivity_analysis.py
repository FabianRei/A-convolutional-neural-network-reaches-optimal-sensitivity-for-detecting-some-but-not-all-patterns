...
def get_csv_column(csv_path, col_name, sort_by=None, exclude_from=None):
    '''
    Simple funciton to extract column of csv tabe. Used to extract data from results.csv file.
...
def get_contrast_sensitivity(experiment, target_d=1.5, shift=False, angle=False):
    '''
    ...
    values.
    :param experiment: Path to where experiment results are
    :param target_d:
    :param shift:
    :param angle:
    :return:
...
if __name__ == "__main__":
    folder_path = os.path.join(on_root_path(), 'local', 'experiment', 'faces')
